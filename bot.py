...
keyboard1.row ('Описание','Ссылки и контакты')
keyboard1.row ('Каталог программ')
keyboard1.row ('Узнать погоду','Наборы стикеров')
keyboard1.row ('| ТЕХНИЧЕСКАЯ ПОДДЕРЖКА |', 'Самописные программы')
...
def send_text(message):
    ...
    if message.text.lower() == "пока":
       ...
       bot.send_document(message.chat.id , st3)
        ...
        observation = mgr.weather_at_place("Волгоград")
        ...
        bot.send_message(message.chat.id , "Ошибочка вышла :(  , попробуйте ещё раз " , reply_markup = keyboard1)
